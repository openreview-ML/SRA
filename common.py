...
class Evaluator:
    ...
    def score_manifold_model(self, scores, obj_truth, bias = 0.0, topk = 5):
        ...
        scores = torch.stack(
            [scores[(attr,obj)] for attr, obj in self.testloader.dataset.pairs], 1
        ) # (Batch, #pairs)
        orig_scores = scores.clone()
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
        ...
        for i in range(len(attr_truth)):
            if pairs[i] in self.testloader.dataset.train_pairs:
                seen_ind.append(i)
            else:
                unseen_ind.append(i)
        ...
        base_scores = torch.stack(
            [allpred[(attr,obj)] for attr, obj in self.testloader.dataset.pairs], 1
        ) # (Batch, #pairs)
